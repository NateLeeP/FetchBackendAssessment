...
describe("Test to see if mocha works", () => {
  it("Checks to see if mocha works", () => {
    var three = 3;
...
describe("Test example data returns expected results", () => {
  ...
  it("Should add to the transaction array", (done) => {
    ...
    .then(({data}) => {
      // Update time stamp to mirror example? Maybe, maybe not
      expect(isEqual(data, [
  ...
  it("Should return object array with payer + points spent when spent route called", (done) =>{
  ...
  it("Should return new points balances after spend call", (done) => {
    axios.get('http://localhost:3000/balance')
    .then(({data}) => {
